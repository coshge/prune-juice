...
fn destabilises(
    ...
    if r.kind == ResourceKind::Container {
        ...
        // A non-empty writable layer holds data that lives in no volume — and
        // an *unmeasured* layer is not an empty one. `/system/df` is the only
        // call that measures it, so with sizing off, or on a daemon that does
        // not report container sizes, the honest answer is "cannot be proven
        // empty". Treating unknown as zero here is how a container holding a
        // database dump reaches the no-confirmation tier.
        match r.size_rw {
            None => {
                rs.record(&subject, "size_rw", "unmeasured");
                return Some(
                    "its writable layer was not measured, so it cannot be proven empty".into(),
                );
            }
            Some(rw) => {
...
fn reversibility_of(a: &Attributed, rs: &mut ReadSet) -> Reversibility {
    ...
    match r.kind {
        ...
        ResourceKind::Container => match r.size_rw {
            ...
            // Unmeasured. `Unknown` is not a soft `No`: a writable layer
            // nobody measured is assumed to hold something, so the price of
            // removing it is stated as permanent rather than free.
            None => {
                rs.record(&subject, "size_rw", "unmeasured");
                Reversibility::Gone
            }
...
mod tests {
    ...
    fn container(name: &str, state: ContainerState, mounts: &[&str]) -> ResourceSummary {
        ...
        // "unmeasured", and an unmeasured layer is never assumed empty.
    ...
    #[test]
    fn a_container_with_an_unmeasured_writable_layer_is_not_free() {
        // `--no-sizes`, or a daemon that does not report container sizes.
        // Treating the missing figure as zero is how a container holding a
        // database dump reaches the tier that deletes without asking.
        let mut c = container("unmeasured", ContainerState::Exited, &[]);
        c.size_rw = None;
        let rep = report(vec![attributed(c)]);
        let v = classify_one(&rep, 0);
        assert_ne!(v.tier, Tier::Free);
        assert_eq!(v.reversibility, Reversibility::Gone);
        assert!(
            v.because.contains("not measured"),
